...
class Board:
    '''Главный класс. Реализовывает взаимодействие между фигурами.
    Создает и завершает игры. создает экземпляры классов фигур и работает с ними вместе
    '''
    ...
    def move_piece(self, row, col, row1, col1):
        ...
        if piece.can_move(self, row, col, row1, col1):
            if king_is_under_attack or self.king_will_under_attack_move(
                    row, col, row1, col1):
                if self.antishah(row, col, row1, col1):
                    return self.move_piece_change(row, col, row1, col1, piece)
                else:
                    raise KingAttack('Невозможный ход, Король будет атакован!')
            else:
                return self.move_piece_change(row, col, row1, col1, piece)
        elif piece.can_attack(self, row, col, row1, col1):
            if king_is_under_attack or self.king_will_under_attack(
                    row, col, row1, col1):
                if self.antishah(row, col, row1, col1):
                    return self.move_piece_change(row, col, row1, col1, piece)
                else:
                ...
                return self.move_piece_change(row, col, row1, col1, piece)
        raise Exception('Фигура не может так ходить')
            ...
            for col in range(8):
                ...
                if piece is None:
                    ...
                    for row1 in range(7, -1, -1):
                        for col1 in range(8):
                            if piece.can_move(self, row, col, row1, col1):
                                if self.king_is_under_attack() or self.king_will_under_attack_move(row, col, row1, col1):
                                    if self.antishah(row, col, row1, col1):
                                        step.append(True)
                                    else:
                                        step.append(False)
    ...
    def move_and_promote_pawn(self, row, col, row1, col1, char):
        if self.char(row, col) == 'P':
            ...
            if (self.color == 1 and row1 != 7) or (
                    self.color == 2 and row1 != 0):
                return False
    ...
    def king_is_under_attack(self):
        for row in range(7, -1, -1):
            for col in range(8):
                if self.char(row, col) == 'K' and self.color == self.field_color(
                        row, col):
...
class Pawn:
    ...
    def can_attack(self, board, row, col, row1, col1):
        ...
        if row + direction == row1 and (col + 1 == col1 or col - 1 == col1):
            if (not (board.get_piece(row1, col1) is None) and board.cell(
                    row1, col1)[0] != board.cell(row, col)[0]):
                board.TACKING_PAWN_PASS = None
                return True
            if (board.get_piece(row1, col1) is None
                ...
                if board.field[row][col1] and [
                        row, col1] == board.TACKING_PAWN_PASS:
