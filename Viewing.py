...
class Ui_Dialog(QWidget):
    ...
    def start_program(self):
        ...
        y = size[1]
        self.label_choice.setGeometry(
            ((self.size[0] // 2) - (size[0] // 2)), y, size[0], size[1])
        self.label_choice.setAlignment(QtCore.Qt.AlignCenter)
        y += size[1] * 2

        self.chess_choice = QPushButton('Шахматы (на 2 игрока)', self)
        self.chess_choice.setGeometry(
            ((self.size[0] // 2) - (size[0] // 2)), y, size[0], size[1])
        y += size[1] * 2

        self.game_choice = QPushButton('Выбрать партию', self)
        self.game_choice.setGeometry(
            ((self.size[0] // 2) - (size[0] // 2)), y, size[0], size[1])

        self.exit_bt = QPushButton('Закрыть игру', self)
        self.exit_bt.setGeometry(
            ((self.size[0] // 2) - (size[0] // 2)), self.size[1] - size[1], size[0], size[1])

        self.table = QListWidget(self)
        self.table.setGeometry(((self.size[0] //
                                 2) +
                                (size[0] //
                                 3)), (min(self.size) *
                                       0.7) //
        ...
        self.table_btn1.setGeometry(((self.size[0] // 2) + (size[0] // 3)), (min(
            self.size) * 0.7 + self.size[1] * 0.1), size[0] * 0.5, size[1] * 0.5)
        self.table_btn1.setVisible(False)

        self.table_btn2 = QPushButton('Удалить', self)
        self.table_btn2.setGeometry(
            ((self.size[0] // 2) + (
                size[0] // 3) + size[0] * 0.7),
            (min(
                self.size) * 0.7 + self.size[1] * 0.1),
            size[0] * 0.5,
            size[1] * 0.5)
    ...
    def draw_chess(self):
        ...
        for i in range(7, -1, -1):
            ...
            for j in range(8):
                ...
                # btn.setIcon(QtGui.QIcon('img\chess1.png'))
                #btn.setIconSize(QtCore.QSize(75, 75))
